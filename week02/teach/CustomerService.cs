...
public class CustomerService {
    ...
    private void AddNewCustomer() {
        ...
        if (_queue.Count >= _maxSize) {                    // Fixed: Use >= instead of >
    ...
    private void ServeCustomer() {
        if (_queue.Count <= 0) {                           // Fixed: Check for empty queue
            Console.WriteLine("No Customers in the queue");
            return;
        }

        // Need to read the customer BEFORE removing it from the list
        var customer = _queue[0];                          // Fixed: Read first
