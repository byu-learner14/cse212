...
public static class Algorithms {
    ...
    private static int Algorithm1(int size) {
        // Plan: Single for loop → should be O(n)
        var count = 0;
    ...
    private static int Algorithm2(int size) {
        // Plan: Nested for loops (both run 'size' times) → should be O(n^2)
        var count = 0;
        for (var i = 0; i < size; ++i)
    ...
    private static int Algorithm3(int size) {
        // Plan: While loop that repeatedly halves the search space 
        // (start moves to middle+1 each time) → should be O(log n)
        var count = 0;
        var start = 0;
        var end = size - 1;
        while (start <= end) {
